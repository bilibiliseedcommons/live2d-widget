...
function loadWidget(waifuPath, apiPath) {
	...
	function registerEventListener() {
		...
		devtools.toString = () => {
			showMessage("哈哈，你打開了控製臺，是想要看看我的小秘密嗎？", 6000, 9);
		};
	...
	function welcomeMessage() {
		...
		if (location.pathname === "/") { // 如果是主頁
			...
			if (location.hostname == referrer.hostname) text = `歡迎觀看<span>「${document.title.split(" - ")[0]}」</span>`;
			...
			else if (domain == "google") text = `Hello！來自 穀歌搜索 的朋友<br>歡迎閱讀<span>「${document.title.split(" - ")[0]}」</span>`;
			else text = `Hello！來自 <span>${referrer.hostname}</span> 的朋友`;
		} else {
			text = `歡迎觀看<span>「${document.title.split(" - ")[0]}」</span>`;
		}
	...
	var userAction = false,
		...
		messageTimer = null,
		messageArray = ["好久不見，日子過得好快呢……", "大壞蛋！你都多久冇理人家了呀，嚶嚶嚶～", "嗨～快來逗我玩吧！", "拿小拳拳錘你胸口！", "記得把小家加入 Adblock 白名單哦！"];
	...
	function loadRandModel() {
		...
		fetch(`${apiPath}/rand_textures/?id=${modelId}-${modelTexturesId}`)
			.then(response => response.json())
			.then(result => {
				if (result.textures.id == 1 && (modelTexturesId == 1 || modelTexturesId == 0)) showMessage("我還冇有其他衣服呢！", 4000, 10);
